...
RIAKC_API std::unique_ptr<basic_client> new_unique_client(const std::string& host,
        const std::string& port, const protocol protocol)
{
    //return std::make_unique<pbc::pbc_client>(host, port);
    return std::unique_ptr<basic_client>(new pbc::pbc_client(host, port));
}
